...
pub extern "C" fn main() -> ! {
    ...
    // HACK: This is a workaround for not being able to use `Iterator::collect` in
    //       userspace. See https://github.com/rtthw/rhubarb/issues/2 for more.
    let mut virtio_inputs = Vec::with_capacity(2);
    virtio_inputs.extend(
        pci::enumerate_devices()
            .into_iter()
            .filter(|dev| dev.vendor_id == 0x1af4 && dev.device_id == 0x1040 + 18)
            .filter_map(|pci_device| virtio_input::Device::new(pci_device).ok()),
    );
